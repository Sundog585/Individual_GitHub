...
public class Test_LoadingScene : MonoBehaviour
{
    ...
    AsyncOperation async;                   // 비동기씬 로딩의 진행상황과 결과를 알 수 있는 클래스

    WaitForSeconds waitForSeconds;          // loadingText 점찍기 간격 유지 설정용
    ...
    PlayerInputSystem inputActions;         // 로딩 완료 후 넘어가기 위한 입력
    ...
    private void Start()
    {
        waitForSeconds = new WaitForSeconds(0.3f);  // Loading ... 시간간격 0.2초
        loadingTextCoroutine = LoadingTestProgress();   // Loading ... 용 코루틴 저장
        StartCoroutine(loadingTextCoroutine);           // 코루팅 실행
    ...
    IEnumerator LoadingTestProgress()
    {
        ...
        while (true)
        {
            ...
            yield return waitForSeconds;    // 정해진 시간동안 대기
            point++;                        // point값 증가
